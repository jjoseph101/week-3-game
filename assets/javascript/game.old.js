...
// NOTES TO SELF
// 1. Need to go DRY re for loops written long ways
...
var picBank = ["rott.gif", "chi.gif", "labdl.gif", "pug2.gif", "jt.gif", "pomm.gif", "mutt.gif", "dal.gif", "husky.gif", "boxer.gif"];
...
//counter to test if word solved//
var numCount=0;
		...
		for (i=0; i<spacedOut.length; i++){
			if (guess==spacedOut[i]) {
				...
				if (begBlank[i]==spacedOut[i]) {
						...
						if (numRem<1) {
							...
							document.getElementById("winCount").innerHTML = totalWins;
							document.getElementById("picLeft").innerHTML = "<img src='assets/images/" + picBank[ranNum] + "'>";
			...
			if (guess != goodLetters[0] && guess != goodLetters[1] && guess != goodLetters[2] && guess != goodLetters[3] && guess != goodLetters[4] && guess != goodLetters[5] && guess != goodLetters[6] && guess != goodLetters[7] && guess != goodLetters[8] && guess != goodLetters[9]) {
				...
				if (guessLeft<1) {
					...
					document.getElementById("picLeft").innerHTML = '<img src="assets/images/slap.gif">';
				} else {};
			};	
		};
